...
const scriptSecurityManager = Services.scriptSecurityManager.getSystemPrincipal();
const dnsService =  Cc["@mozilla.org/network/dns-service;1"].getService(Ci.nsIDNSService); // https://developer.mozilla.org/en-US/docs/Mozilla/Tech/XPCOM/Reference/Interface/nsIDNSService
...
const ipResolver = {
    resolveIPs: () => {
        ...
        return new Promise( (resolve, reject) => {
            if(addresses.length > 1)
                ...
                ipResolver._forceGetIPs(resolve);
        });
    },
    _forceGetIPs: (resolve) =>{
        ...
        var message = new Uint8Array([].map.call("get my ipaddresses", i => i.charCodeAt(0)));
